...
public class KhoaService implements ManageKhoaUseCase {
    ...
    @Override
    public List<Khoa> getAllKhoa() { return khoaPort.layTatCa(); }

    @Override
    public Khoa getKhoaById(Long id) {
        return khoaPort.timTheoId(id).orElseThrow(() -> new RuntimeException("Không tìm thấy khoa ID: " + id));
    }
    ...
    public Khoa updateKhoa(Long id, Khoa details) {
        ...
        if (details.getTenKhoa() == null || details.getTenKhoa().trim().isEmpty()) throw new RuntimeException("Tên khoa không được để trống");
        khoa.setTenKhoa(details.getTenKhoa().trim());
        return khoaPort.luu(khoa);
    }

    @Override
    public void deleteKhoa(Long id) { khoaPort.xoa(id); }
