...
public class MonHocService implements ManageMonHocUseCase {
    ...
    public MonHoc updateMonHoc(Long id, MonHoc details) {
        ...
        if (details.getTenMonHoc() == null || details.getTenMonHoc().trim().isEmpty()) {
            throw new RuntimeException("Tên môn học không được để trống");
        }
    ...
    public void importExcel(InputStream inputStream) {
        try (Workbook workbook = new XSSFWorkbook(inputStream)) {
            ...
            for (Row row : sheet) {
                ...
                if (tenMonHoc == null || tenMonHoc.trim().isEmpty()) continue;
                ...
                monHoc.setTenMonHoc(tenMonHoc.trim());
                monHoc.setTinChi(tinChi);
                monHoc.setKhoa(khoa);
